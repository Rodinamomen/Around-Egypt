...
@HiltViewModel
internal class HomeViewModel @Inject constructor(private val getExperiencesUC: GetExperiencesUC,private val getRecommendedExperiencesUC: GetRecommendedExperiencesUC,private val likeExperienceUC: LikeExperienceUC,private val searchUC: SearchUC) : ViewModel(){
    ...
    private val _recommendExperienceState = MutableStateFlow<Resource<Experiences>>(Resource.Loading())
    ...
    fun getExperiences(){
        viewModelScope.launch {
            getExperiencesUC.invoke().collect{
                resource->
                when(resource){
    ...
    fun getRecommendedExperiences(){
        viewModelScope.launch {
            getRecommendedExperiencesUC.invoke().collect{
                    resource->
                when(resource){
    ...
    fun likeExperience(id:String){
        viewModelScope.launch {
            likeExperienceUC.invoke(id).collect {
            resource->
            when (resource) {
